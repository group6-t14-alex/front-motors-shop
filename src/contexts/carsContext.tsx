...
interface carProviderData {
    ...
    getAllAvailableCars: () => Promise<void>
}
...
export const CarProvider = ({children}: Props) => {
    ...
    const getAllAvailableCars = async () => {
        ...
        const avaiableCars = response.data.filter((availableCar: { isActive: boolean; }) => availableCar.isActive === true)        
        setFiltredCars(avaiableCars);
    };
    ...
    return (
        ...
        getCarsByFuel, getCarsByYear, getCarsByKm, getCarsByPrice, editAd, deleteAd, car, setCar, getAllAvailableCars,
        }}>
            {children}
        </CarContext.Provider>
    )
